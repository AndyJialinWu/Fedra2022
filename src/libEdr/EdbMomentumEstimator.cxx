...
float EdbMomentumEstimator::PMSang(EdbTrackP &tr)
{
  ...
  //eF1X = MCSErrorFunction("eF1X",x0,dtx);
  //eF1X = new TF1("eF1X", Form("sqrt(214.3296*x/%f*((1+0.038*log(x/(%f)))**2)/([0])**2+%e)",x0,x0,dtx));
  eF1X = new TF1("eF1X", MCSAngErrFunc, 0.0, 1.0, 3, 1);    
  eF1X->SetRange(0,Min(14,maxX));
  eF1X->SetParameter(0,2000.);                             // starting value for momentum in MeV

  eF1X->FixParameter(1,x0);
  eF1X->FixParameter(2,dtx);

  //eF1Y = MCSErrorFunction("eF1Y",x0,dty);    
  //eF1Y = new TF1("eF1Y", Form("sqrt(214.3296*x/%f*((1+0.038*log(x/(%f)))**2)/([0])**2+%e)",x0,x0,dty));
  eF1Y = new TF1("eF1Y", MCSAngErrFunc, 0.0, 1.0, 3, 1);
  eF1Y->SetRange(0,Min(14,maxY));
  eF1Y->SetParameter(0,2000.);                             // starting value for momentum in MeV

  eF1Y->FixParameter(1,x0);
  eF1Y->FixParameter(2,dty);

  //eF1 = MCSErrorFunction("eF1",x0,dt);     
  //eF1 = new TF1("eF1", Form("sqrt(214.3296*x/%f*((1+0.038*log(x/(%f)))**2)/([0])**2+%e)",x0,x0,dt));
  eF1 = new TF1("eF1", MCSAngErrFunc, 0.0, 1.0, 3, 1);
  eF1->SetRange(0,Min(14,max3D));
  eF1->SetParameter(0,2000.);                              // starting value for momentum in MeV

  eF1->FixParameter(1,x0);
  eF1->FixParameter(2,dt);
...
float EdbMomentumEstimator::PMScoordinate(EdbTrackP &tr)
{
  ...
  */
  ...
  //eF1X = new TF1("eF1X", Form("sqrt(([1])**2+(2./3)*((x*sqrt(1+%f**2))**3)*(0.0136**2)*[0]/%f)", tmean, eX0));
  eF1X = new TF1("eF1X", MCSCoordErrFunc, 0.0, 1.0, 4, 1);
  ...
  eF1X->FixParameter(2, tmean);
  eF1X->FixParameter(3, eX0);
  
  //eF1Y = MCSCoordErrorFunction("eF1Y",tmean,eX0);
  //eF1Y = new TF1("eF1Y", Form("sqrt(([1])**2+(2./3)*((x*sqrt(1+%f**2))**3)*(0.0136**2)*[0]/%f)", tmean, eX0)); 
  //eF1Y->SetRange(0,Min(57,maxY));
  eF1Y = new TF1("eF1Y", MCSCoordErrFunc, 0.0, 1.0, 4, 1);
  ...
  eF1Y->FixParameter(2, tmean);
  eF1Y->FixParameter(3, eX0);
  
  //eF1 = MCSCoordErrorFunction("eF1",tmean,eX0);
  //eF1 = new TF1("eF1", Form("sqrt(([1])**2+(2./3)*((x*sqrt(1+%f**2))**3)*(0.0136**2)*[0]/%f)", tmean, eX0));
  //eF1->SetRange(0,Min(57,max3D));
  eF1 = new TF1("eF1", MCSCoordErrFunc, 0.0, 1.0, 4, 1);
  ...
  ePx = 1./TMath::Sqrt(eF1X->GetParameter(0));
  ePy = 1./TMath::Sqrt(eF1Y->GetParameter(0));
...
float EdbMomentumEstimator::CellWeight(int npl, int m)
{
  ...
  return 2.*TMath::Sqrt( npl/m + 1./m/m*( npl*(m-1) - m*(m-1)/2.) );
  // return 1;
}
...
TF1 *EdbMomentumEstimator::MCSErrorFunction(const char *name, float x0, float dtx)
{
  ...
  // return new TF1(name,Form("sqrt((214.3296*x/%f) * ((1+0.038*log(x/(%f))) * (1+0.038*log(x/(%f))))/([0]*[0]) + %e)",x0, x0, x0, dtx));
...
TF1 *EdbMomentumEstimator::MCSCoordErrorFunction(const char *name, float tmean,float x0)
{
  ...
  // return new TF1(name,Form("sqrt(pow([1],2)+(2./3)*pow(x*sqrt(1+pow(%f,2)),3)*(0.0136*0.0136)*[0]/%f)",tmean, x0));
...
double EdbMomentumEstimator::MCSAngErrFunc(double *x, double *p){
  // p[0]: momentum in MeV/c, p[1]: x0 radiation length in mm, p[2]: tan theta measurement error

  double error = TMath::Sqrt( (214.3296*x[0]/p[1]) * TMath::Power(1.000+0.038*TMath::Log(x[0]/p[1]), 2) * (1.00/TMath::Power(p[0], 2)) + p[2]);
  return error;
...
//______________________________________________________________________________________
double EdbMomentumEstimator::MCSCoordErrFunc(double *x, double *p){
  // p[0]: 1/momentum^2, p[1]: coordinate error, p[2]: tan theta mean, p[3]: eX0 radiation length in um
...
int EdbMomentumEstimator::PMSang_base(EdbTrackP &tr)
{
  ...
  float initP = 1., minP=0., maxP=2000.;                             // starting value for momentum in GeV
...
TF1 *EdbMomentumEstimator::MCSErrorFunction_base(const char *name, float x0, float dtx)
{
  ...
  //return new TF1(name,Form("sqrt(0.0002143296*x/%f*(pow(1+0.038*log(x/(%f)),2))/([0]*[0])+%f)",x0, x0, dtx));
